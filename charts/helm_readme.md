...
Template Directive (dazu zählen auch Variablen) in Helm Charts werden durch doppelte Mengenklammern eingeschlossen angegeben also zwischen `{{` und `}}` z.B: `{{ <var-name> }}`. Dabei ist zu beachten, dass ***nur definierte Variablen*** verwendet werden dürfen. Wenn eine Variable nicht definiert ist führt das zu einem Fehler beim Rendern des Charts.
